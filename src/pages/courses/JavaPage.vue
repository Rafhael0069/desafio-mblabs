<template>
  <v-container>
    <v-row dense>
      ...
      >
      <v-card>
          <v-row class="ma-0 pa-0" @click="expand(index)">
            <v-col cols="4" class="pa-3">
              <v-img :src="item.image" height="90" width="90"> </v-img>
            </v-col>
            <v-col cols="8">
              <h3>{{ item.titulo }} - {{ item.plataforma }}</h3>
              <p>{{ item.resumo }}</p>
            </v-col>
          ...
          <v-card-actions>
            <v-btn block class="primary"> ir para pagina do curso </v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    </v-row>
